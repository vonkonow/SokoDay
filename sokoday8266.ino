...
#include <NeoPixelBrightnessBus.h>  // https://github.com/Makuna/NeoPixelBus/blob/master/src/NeoPixelBrightnessBus.h
...
const int brightness = 64;                    // adjust to brighten or darken display
...
void setup()
{
  ...
  Serial.println("WS2812 SokoDay 8266 serial debug initiated.");
  ...
  if ((WiFiMulti.run() == WL_CONNECTED)) {
    ...
    String addr = "http://www.vonkonow.com/sokoday/";
    if (difficulty == 0) {
      addr += "short.php";
    }
    if (difficulty == 1) {
    ...
    Serial.println(addr);
    if (http.begin(client, addr)) {  // HTTP
...
void testWin() {
  ...
  if (nrGoals > 0) {
    // we have a winner - post result
    WiFiClient client;
    HTTPClient http;
    String addr = "http://vonkonow.com/sokoday/";
    if (difficulty == 0) {
      addr += "short.php?g=1&m=";
    }
    if (difficulty == 1) {
      addr += "medium.php?g=1&m=";
    }
    if (difficulty == 2) {
      addr += "long.php?g=1&m=";
    }
